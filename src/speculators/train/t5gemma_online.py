...
class T5GemmaHiddenStateExtractor:
    ...
    def extract_stacked(
        ...
        encoder_ids = encoder_input_ids.long().unsqueeze(0).to(self.device)
        decoder_ids = decoder_input_ids.long().unsqueeze(0).to(self.device)
        attention_mask = torch.ones_like(encoder_ids)
        ...
        hidden_states = torch.stack(
            [self._captures[layer_id][0] for layer_id in self.target_layer_ids]
            + [self._captures["last"][0]],
            dim=1,
        ...
        if self.device.type == "cuda" and self._num_extractions % 10 == 0:
            logger.info(
                "Online extraction %d; peak GPU allocated/reserved %.2f/%.2f GiB",
